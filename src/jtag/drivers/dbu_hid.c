...
/***************************************************************************
 *   Copyright (C) 2024 by Gregor Bruce                                    *
 *   gregor@web.com                                                        *
 *                                                                         *
...
static uint16_t dbu_vid[MAX_USB_IDS + 1] = { 0 };
static uint16_t dbu_pid[MAX_USB_IDS + 1] = { 0 };
static int dbu_backend = -1;
...
#define QUEUED_SEQ_BUF_LEN (dbu_handle->packet_usable_size - 3)
...
static struct dbu *dbu_handle;


static int dbu_quit(void);

static int dbu_open(void)
{
	const struct dbu_backend *backend = NULL;

	struct dbu *dap = calloc(1, sizeof(struct dbu));
	if (!dap) {
	...
	if (dbu_backend >= 0) {
		/* Use forced backend */
		backend = dbu_backends[dbu_backend];
		if (backend->open(dap, dbu_vid, dbu_pid, adapter_get_required_serial()) != ERROR_OK)
			backend = NULL;
	} else {
		/* Try all backends */
		for (unsigned int i = 0; i < ARRAY_SIZE(dbu_backends); i++) {
			backend = dbu_backends[i];
			if (backend->open(dap, dbu_vid, dbu_pid, adapter_get_required_serial()) == ERROR_OK)
				break;
			else
				backend = NULL;
	...
	if (!backend) {
		LOG_ERROR("unable to find a matching dbu device");
		free(dap);
		return ERROR_FAIL;
	}
...
static void dbu_close(struct dbu *dap)
{
	if (dap->backend) {
...
static void dbu_flush_read(struct dbu *dap)
{
	unsigned int i;
	/* Some dbu adapters keep buffered packets over
...
static int dbu_xfer(struct dbu *dap, int txlen)
{
	...
	if (resp[0] == DAP_ERROR) {
		LOG_ERROR("dbu command 0x%" PRIx8 " not implemented", current_cmd);
		return ERROR_NOT_IMPLEMENTED;
	}

	if (resp[0] != current_cmd) {
		LOG_ERROR("dbu command mismatch. Sent 0x%" PRIx8
...
static int dbu_cmd_dap_swj_pins(uint8_t pins, uint8_t mask, uint32_t delay, uint8_t *input)
{
	uint8_t *command = dbu_handle->command;
	...
	int retval = dbu_xfer(dbu_handle, 7);
	if (retval != ERROR_OK) {
		LOG_ERROR("dbu command CMD_DAP_SWJ_PINS failed.");
		return ERROR_JTAG_DEVICE_ERROR;
	}

	if (input)
		*input = dbu_handle->response[1];

	return ERROR_OK;
}

static int dbu_cmd_dap_swj_clock(uint32_t swj_clock)
{
	uint8_t *command = dbu_handle->command;
	...
	int retval = dbu_xfer(dbu_handle, 5);
	if (retval != ERROR_OK || dbu_handle->response[1] != DAP_OK) {
		LOG_ERROR("dbu command CMD_DAP_SWJ_CLOCK failed.");
		return ERROR_JTAG_DEVICE_ERROR;
	}
...
static int dbu_cmd_dap_swj_sequence(uint8_t s_len, const uint8_t *sequence)
{
	uint8_t *command = dbu_handle->command;

#ifdef FIZZIT_JTAG_DEBUG
	...
	int retval = dbu_xfer(dbu_handle, 2 + DIV_ROUND_UP(s_len, 8));
	if (retval != ERROR_OK || dbu_handle->response[1] != DAP_OK)
		return ERROR_FAIL;
...
static int dbu_cmd_dap_info(uint8_t info, uint8_t **data)
{
	uint8_t *command = dbu_handle->command;
	...
	int retval = dbu_xfer(dbu_handle, 2);
	if (retval != ERROR_OK) {
		LOG_ERROR("dbu command CMD_INFO failed.");
		return ERROR_JTAG_DEVICE_ERROR;
	}

	*data = &dbu_handle->response[1];

	return ERROR_OK;
}

static int dbu_cmd_dap_led(uint8_t led, uint8_t state)
{
	uint8_t *command = dbu_handle->command;
	...
	int retval = dbu_xfer(dbu_handle, 3);
	if (retval != ERROR_OK || dbu_handle->response[1] != DAP_OK) {
		LOG_ERROR("dbu command CMD_LED failed.");
		return ERROR_JTAG_DEVICE_ERROR;
	}
...
static int dbu_cmd_dap_connect(uint8_t mode)
{
	uint8_t *command = dbu_handle->command;
	...
	int retval = dbu_xfer(dbu_handle, 2);
	if (retval != ERROR_OK) {
		LOG_ERROR("dbu command CMD_CONNECT failed.");
		return ERROR_JTAG_DEVICE_ERROR;
	}

	if (dbu_handle->response[1] != mode) {
		LOG_ERROR("dbu failed to connect in mode (%d)", mode);
		return ERROR_JTAG_DEVICE_ERROR;
	}
...
static int dbu_cmd_dap_disconnect(void)
{
	uint8_t *command = dbu_handle->command;

	command[0] = CMD_DAP_DISCONNECT;

	int retval = dbu_xfer(dbu_handle, 1);
	if (retval != ERROR_OK || dbu_handle->response[1] != DAP_OK) {
		LOG_ERROR("dbu command CMD_DISCONNECT failed.");
		return ERROR_JTAG_DEVICE_ERROR;
	}
...
static int dbu_cmd_dap_tfer_configure(uint8_t idle, uint16_t retry_count, uint16_t match_retry)
{
	uint8_t *command = dbu_handle->command;
	...
	int retval = dbu_xfer(dbu_handle, 6);
	if (retval != ERROR_OK || dbu_handle->response[1] != DAP_OK) {
		LOG_ERROR("dbu command CMD_TFER_Configure failed.");
		return ERROR_JTAG_DEVICE_ERROR;
	}
...
static int dbu_cmd_dap_swd_configure(uint8_t cfg)
{
	uint8_t *command = dbu_handle->command;
	...
	int retval = dbu_xfer(dbu_handle, 2);
	if (retval != ERROR_OK || dbu_handle->response[1] != DAP_OK) {
		LOG_ERROR("dbu command CMD_SWD_Configure failed.");
		return ERROR_JTAG_DEVICE_ERROR;
	}
...
#if 0
static int dbu_cmd_dap_delay(uint16_t delay_us)
{
	uint8_t *command = dbu_handle->command;
	...
	int retval = dbu_xfer(dbu_handle, 3);
	if (retval != ERROR_OK || dbu_handle->response[1] != DAP_OK) {
		LOG_ERROR("dbu command CMD_Delay failed.");
		return ERROR_JTAG_DEVICE_ERROR;
	}
...
static int dbu_metacmd_targetsel(uint32_t instance_id)
{
	uint8_t *command = dbu_handle->command;
	...
	int retval = dbu_xfer(dbu_handle, idx);
	if (retval != ERROR_OK || dbu_handle->response[1] != DAP_OK) {
		LOG_ERROR("dbu command SWD_Sequence failed.");
		return ERROR_JTAG_DEVICE_ERROR;
	}
 ...
 */
static int dbu_cmd_dap_swo_transport(uint8_t transport)
{
	uint8_t *command = dbu_handle->command;
	...
	int retval = dbu_xfer(dbu_handle, 2);
	if (retval != ERROR_OK || dbu_handle->response[1] != DAP_OK) {
		LOG_ERROR("dbu: command CMD_SWO_Transport(%d) failed.", transport);
		return ERROR_JTAG_DEVICE_ERROR;
	}
 ...
 */
static int dbu_cmd_dap_swo_mode(uint8_t mode)
{
	uint8_t *command = dbu_handle->command;
	...
	int retval = dbu_xfer(dbu_handle, 2);
	if (retval != ERROR_OK || dbu_handle->response[1] != DAP_OK) {
		LOG_ERROR("dbu: command CMD_SWO_Mode(%d) failed.", mode);
		return ERROR_JTAG_DEVICE_ERROR;
	}
 ...
 */
static int dbu_cmd_dap_swo_baudrate(
					...
					uint32_t *dev_baudrate)
{
	...
	int retval = dbu_xfer(dbu_handle, 5);
	uint32_t rvbr = le_to_h_u32(&dbu_handle->response[1]);
	if (retval != ERROR_OK || rvbr == 0) {
		LOG_ERROR("dbu: command CMD_SWO_Baudrate(%u) -> %u failed.", in_baudrate, rvbr);
		if (dev_baudrate)
			*dev_baudrate = 0;
		return ERROR_JTAG_DEVICE_ERROR;
	}
 ...
 *                          not yet been read.
 */
static int dbu_cmd_dap_swo_control(uint8_t control)
{
	uint8_t *command = dbu_handle->command;
	...
	int retval = dbu_xfer(dbu_handle, 2);
	if (retval != ERROR_OK || dbu_handle->response[1] != DAP_OK) {
		LOG_ERROR("dbu: command CMD_SWO_Control(%d) failed.", control);
		return ERROR_JTAG_DEVICE_ERROR;
	}
 ...
 */
static int dbu_cmd_dap_swo_status(
					...
					size_t *trace_count)
{
	...
	int retval = dbu_xfer(dbu_handle, 1);
	if (retval != ERROR_OK) {
		LOG_ERROR("dbu: command CMD_SWO_Status failed.");
		return ERROR_JTAG_DEVICE_ERROR;
	}

	if (trace_status)
		*trace_status = dbu_handle->response[1];
	if (trace_count)
		*trace_count = le_to_h_u32(&dbu_handle->response[2]);
 ...
 */
static int dbu_cmd_dap_swo_data(
					...
					uint8_t *data)
{
	...
	int retval = dbu_xfer(dbu_handle, 3);
	if (retval != ERROR_OK) {
		LOG_ERROR("dbu: command CMD_SWO_Data failed.");
		return ERROR_JTAG_DEVICE_ERROR;
	}

	*trace_status = dbu_handle->response[1];
	*trace_count = le_to_h_u16(&dbu_handle->response[2]);

	if (*trace_count > 0)
		memcpy(data, &dbu_handle->response[4], *trace_count);

	return ERROR_OK;
}

static void dbu_swd_discard_all_pending(struct dbu *dap)
{
	for (unsigned int i = 0; i < MAX_PENDING_REQUESTS; i++)
...
static void dbu_swd_cancel_transfers(struct dbu *dap)
{
	dap->backend->cancel_all(dap);
	dbu_flush_read(dap);
	dbu_swd_discard_all_pending(dap);
}

static void dbu_swd_write_from_queue(struct dbu *dap)
{
	...
	LOG_DEBUG_IO("Executing %d queued transactions from FIFO index %u%s",
				 ...
				 dbu_handle->swd_cmds_differ ? "" : ", same swd ops");
	...
	bool block_cmd = !dbu_handle->swd_cmds_differ
	...
	for (unsigned int i = 0; i < block->transfer_count; i++) {
		 ...
		 * dbu_cmd_dap_tfer_configure() and
		 * dbu_cmd_dap_swd_configure() in
		 * dbu_init().
		 */
		if (!(cmd & SWD_CMD_RNW) &&
		    !(cmd & SWD_CMD_APNDP) &&
...
static void dbu_swd_read_process(struct dbu *dap, enum dbu_blocking blocking)
{
	int retval;
	...
	if (timeout && blocking == FIZZIT_NON_BLOCKING)
		return;
	...
	if (resp[0] != block->command) {
		LOG_ERROR("dbu command mismatch. Expected 0x%x received 0x%" PRIx8,
			block->command, resp[0]);
		dbu_swd_cancel_transfers(dap);
		queued_retval = ERROR_FAIL;
		return;
	}
	...
	if (resp[idx] & 0x08) {
		LOG_DEBUG("dbu Protocol Error @ %d (wrong parity)", transfer_count);
		queued_retval = ERROR_FAIL;
		goto skip;
	}
	...
	if (block->transfer_count != transfer_count) {
		LOG_ERROR("dbu transfer count mismatch: expected %d, got %d",
...
static int dbu_swd_run_queue(void)
{
	if (dbu_handle->write_count + dbu_handle->read_count) {
		if (dbu_handle->pending_fifo_block_count)
			dbu_swd_read_process(dbu_handle, FIZZIT_NON_BLOCKING);

		dbu_swd_write_from_queue(dbu_handle);
	}

	while (dbu_handle->pending_fifo_block_count)
		dbu_swd_read_process(dbu_handle, FIZZIT_BLOCKING);

	dbu_handle->pending_fifo_put_idx = 0;
	dbu_handle->pending_fifo_get_idx = 0;
...
static unsigned int dbu_tfer_cmd_size(unsigned int write_count,
...
static unsigned int dbu_tfer_resp_size(unsigned int write_count,
							unsigned int read_count, bool block_tfer)
{
	unsigned int size;
	if (block_tfer)
...
static void dbu_swd_queue_cmd(uint8_t cmd, uint32_t *dst, uint32_t data)
{
	/* TARGETSEL register write cannot be queued */
	if (swd_cmd(false, false, DP_TARGETSEL) == cmd) {
		queued_retval = dbu_swd_run_queue();

		dbu_metacmd_targetsel(data);
		return;
	}

	/* Compute sizes of the DAP Transfer command and the expected response
	 * for all queued and this operation */
	unsigned int write_count = dbu_handle->write_count;
	unsigned int read_count = dbu_handle->read_count;
	bool block_cmd;
	if (write_count + read_count < CMD_DAP_TFER_BLOCK_MIN_OPS)
		block_cmd = false;
	else
		block_cmd = !dbu_handle->swd_cmds_differ
					&& cmd == dbu_handle->common_swd_cmd;
	...
	unsigned int cmd_size = dbu_tfer_cmd_size(write_count, read_count,
													block_cmd);
	unsigned int resp_size = dbu_tfer_resp_size(write_count, read_count,
													block_cmd);
	...
	if (cmd_size > tfer_max_command_size
		...
		unsigned int packet_count = dbu_handle->quirk_mode ? 1 : dbu_handle->packet_count;
		if (dbu_handle->pending_fifo_block_count >= packet_count)
			dbu_swd_read_process(dbu_handle, FIZZIT_BLOCKING);
	}

	assert(dbu_handle->pending_fifo[dbu_handle->pending_fifo_put_idx].transfer_count < pending_queue_len);

	if (queued_retval != ERROR_OK)
		return;

	struct pending_request_block *block = &dbu_handle->pending_fifo[dbu_handle->pending_fifo_put_idx];
	struct pending_transfer_result *transfer = &(block->transfers[block->transfer_count]);
	transfer->data = data;
	transfer->cmd = cmd;
	if (block->transfer_count == 0) {
		dbu_handle->swd_cmds_differ = false;
		dbu_handle->common_swd_cmd = cmd;
	} else if (cmd != dbu_handle->common_swd_cmd) {
		dbu_handle->swd_cmds_differ = true;
	}

	if (cmd & SWD_CMD_RNW) {
		/* Queue a read transaction */
		transfer->buffer = dst;
		dbu_handle->read_count++;
	} else {
		dbu_handle->write_count++;
	}
	block->transfer_count++;
}

static void dbu_swd_write_reg(uint8_t cmd, uint32_t value, uint32_t ap_delay_clk)
{
	assert(!(cmd & SWD_CMD_RNW));
	dbu_swd_queue_cmd(cmd, NULL, value);
}

static void dbu_swd_read_reg(uint8_t cmd, uint32_t *value, uint32_t ap_delay_clk)
{
	assert(cmd & SWD_CMD_RNW);
	dbu_swd_queue_cmd(cmd, value, 0);
}

static int dbu_get_serial_info(void)
{
	uint8_t *data;

	int retval = dbu_cmd_dap_info(INFO_ID_SERNUM, &data);
	if (retval != ERROR_OK)
		return retval;

	if (data[0]) /* strlen */
		LOG_INFO("dbu: Serial# = %s", &data[1]);

	return ERROR_OK;
}

static int dbu_get_version_info(void)
{
	uint8_t *data;

	/* INFO_ID_FW_VER - string */
	int retval = dbu_cmd_dap_info(INFO_ID_FW_VER, &data);
	if (retval != ERROR_OK)
		return retval;

	if (data[0]) /* strlen */
		LOG_INFO("dbu: FW Version = %s", &data[1]);

	return ERROR_OK;
}

static int dbu_get_caps_info(void)
{
	uint8_t *data;

	/* INFO_ID_CAPS - byte */
	int retval = dbu_cmd_dap_info(INFO_ID_CAPS, &data);
	if (retval != ERROR_OK)
		return retval;
	...
	if (data[0] == 1 || data[0] == 2) {
		...
		dbu_handle->caps = caps;

		for (unsigned int i = 0; i < INFO_CAPS__NUM_CAPS; ++i) {
			if (caps & BIT(i))
				LOG_INFO("dbu: %s", info_caps_str[i]);
		}
	}
...
static int dbu_get_swo_buf_sz(uint32_t *swo_buf_sz)
{
	uint8_t *data;

	/* INFO_ID_SWO_BUF_SZ - word */
	int retval = dbu_cmd_dap_info(INFO_ID_SWO_BUF_SZ, &data);
	if (retval != ERROR_OK)
		return retval;
	...
	*swo_buf_sz = le_to_h_u32(&data[1]);
	...
	LOG_INFO("dbu: SWO Trace Buffer Size = %u bytes", *swo_buf_sz);

	return ERROR_OK;
}

static int dbu_get_status(void)
{
	uint8_t d;

	int retval = dbu_cmd_dap_swj_pins(0, 0, 0, &d);
...
static int dbu_swd_switch_seq(enum swd_special_seq seq)
{
	const uint8_t *s;
	unsigned int s_len;
	int retval;
	...
	if (dbu_handle->quirk_mode && seq != LINE_RESET &&
		...
		dbu_cmd_dap_disconnect();

		/* When we are reconnecting, DAP_Connect needs to be rerun, at
		 * least on Keil ULINK-ME */
		retval = dbu_cmd_dap_connect(CONNECT_SWD);
		if (retval != ERROR_OK)
			return retval;
	}
	...
	retval = dbu_cmd_dap_swj_sequence(s_len, s);
	if (retval != ERROR_OK)
		return retval;

	/* Atmel EDBG needs renew clock setting after SWJ_Sequence
	 * otherwise default frequency is used */
	return dbu_cmd_dap_swj_clock(adapter_get_speed_khz());
}

static int dbu_swd_open(void)
{
	if (!(dbu_handle->caps & INFO_CAPS_SWD)) {
		LOG_ERROR("dbu: SWD not supported");
		return ERROR_JTAG_DEVICE_ERROR;
	}

	int retval = dbu_cmd_dap_connect(CONNECT_SWD);
	if (retval != ERROR_OK)
		return retval;

	/* Add more setup here.??... */

	LOG_INFO("dbu: Interface Initialised (SWD)");
	return ERROR_OK;
}

static int dbu_init(void)
{
	uint8_t *data;
	...
	dbu_flush_read(dbu_handle);

	retval = dbu_get_caps_info();
	if (retval != ERROR_OK)
		return retval;

	retval = dbu_get_version_info();
	if (retval != ERROR_OK)
		return retval;

	retval = dbu_get_serial_info();
	if (retval != ERROR_OK)
		return retval;
	...
	if (swd_mode) {
		...
		if (!(dbu_handle->caps & INFO_CAPS_JTAG)) {
			LOG_ERROR("dbu: JTAG not supported");
			return ERROR_JTAG_DEVICE_ERROR;
		}

		retval = dbu_cmd_dap_connect(CONNECT_JTAG);
		if (retval != ERROR_OK)
			return retval;

		LOG_INFO("dbu: Interface Initialised (JTAG)");
	}

	/* Be conservative and suppress submitting multiple HID requests
	 * until we get packet count info from the adaptor */
	dbu_handle->packet_count = 1;

	/* INFO_ID_PKT_SZ - short */
	retval = dbu_cmd_dap_info(INFO_ID_PKT_SZ, &data);
	if (retval != ERROR_OK)
		goto init_err;

	if (data[0] == 2) {  /* short */
		uint16_t pkt_sz = data[1] + (data[2] << 8);
		if (pkt_sz != dbu_handle->packet_size) {
			dbu_handle->backend->packet_buffer_free(dbu_handle);
			retval = dbu_handle->backend->packet_buffer_alloc(dbu_handle, pkt_sz);
			if (retval != ERROR_OK)
				goto init_err;

			LOG_DEBUG("dbu: Packet Size = %" PRIu16, pkt_sz);
		}
	}
	...
	tfer_max_command_size = dbu_handle->packet_usable_size;
	tfer_max_response_size = dbu_handle->packet_usable_size;
	unsigned int max_reads = tfer_max_response_size / 4;
	pending_queue_len = max_reads + (tfer_max_command_size - max_reads) / 5;
	dbu_handle->write_count = 0;
	dbu_handle->read_count = 0;

	/* INFO_ID_PKT_CNT - byte */
	retval = dbu_cmd_dap_info(INFO_ID_PKT_CNT, &data);
	if (retval != ERROR_OK)
		goto init_err;
	...
	if (data[0] == 1) { /* byte */
		...
		if (pkt_cnt > 1)
			dbu_handle->packet_count = MIN(MAX_PENDING_REQUESTS, pkt_cnt);

		LOG_DEBUG("dbu: Packet Count = %u", pkt_cnt);
	}

	LOG_DEBUG("Allocating FIFO for %u pending packets", dbu_handle->packet_count);
	for (unsigned int i = 0; i < dbu_handle->packet_count; i++) {
		dbu_handle->pending_fifo[i].transfers = malloc(pending_queue_len
									 * sizeof(struct pending_transfer_result));
		if (!dbu_handle->pending_fifo[i].transfers) {
			LOG_ERROR("Unable to allocate memory for dbu queue");
			retval = ERROR_FAIL;
			goto init_err;
		}
	}
	...
	retval = dbu_cmd_dap_swj_clock(adapter_get_speed_khz());
	if (retval != ERROR_OK)
		goto init_err;

	/* Ask dbu to automatically retry on receiving WAIT for
	 * up to 64 times. This must be changed to 0 if sticky
	 * overrun detection is enabled. */
	retval = dbu_cmd_dap_tfer_configure(0, 64, 0);
	if (retval != ERROR_OK)
		goto init_err;
	...
	if (swd_mode) {
		...
		retval = dbu_cmd_dap_swd_configure(0);	/* 1 TRN, no Data Phase */
		if (retval != ERROR_OK)
			goto init_err;
	}
	/* Both LEDs on */
	 ...
	 * without LEDs */
	(void)dbu_cmd_dap_led(LED_ID_CONNECT, LED_ON);
	(void)dbu_cmd_dap_led(LED_ID_RUN, LED_ON);
	...
	if (jtag_reset_config & RESET_CNCT_UNDER_SRST) {
		if (jtag_reset_config & RESET_SRST_NO_GATING) {
			retval = dbu_cmd_dap_swj_pins(0, SWJ_PIN_SRST, 0, NULL);
			if (retval != ERROR_OK)
				goto init_err;
			LOG_INFO("Connecting under reset");
		}
	}
	LOG_INFO("dbu: Interface ready");
	return ERROR_OK;
...
static int dbu_quit(void)
{
	...
	/* Both LEDs off */
	dbu_cmd_dap_led(LED_ID_RUN, LED_OFF);
	dbu_cmd_dap_led(LED_ID_CONNECT, LED_OFF);

	dbu_close(dbu_handle);

	return ERROR_OK;
}

static int dbu_reset(int trst, int srst)
{
	...
	int retval = dbu_cmd_dap_swj_pins(output_pins,
			SWJ_PIN_TRST | SWJ_PIN_SRST, 0, NULL);
	if (retval != ERROR_OK)
		LOG_ERROR("dbu: Interface reset failed");
	return retval;
}

static void dbu_execute_sleep(struct jtag_command *cmd)
{
#if 0
	int retval = dbu_cmd_dap_delay(cmd->cmd.sleep->us);
	if (retval != ERROR_OK)
#endif
...
static int dbu_execute_tlr_reset(struct jtag_command *cmd)
{
	LOG_INFO("cmsis-dap JTAG TLR_RESET");
	uint8_t seq = 0xff;

	int retval = dbu_cmd_dap_swj_sequence(8, &seq);
	if (retval == ERROR_OK)
...
/* Set new end state */
static void dbu_end_state(enum tap_state state)
{
...
static void dbu_flush(void)
{
	...
	int retval = dbu_xfer(dbu_handle, queued_seq_buf_end + 2);

	uint8_t *resp = dbu_handle->response;
	if (retval != ERROR_OK || resp[1] != DAP_OK) {
		LOG_ERROR("dbu command CMD_DAP_JTAG_SEQ failed.");
		exit(-1);
	}
 ...
 */
static void dbu_add_jtag_sequence(unsigned int s_len, const uint8_t *sequence,
...
static void dbu_add_tms_sequence(const uint8_t *sequence, int s_len)
{
	...
	for (int i = 0; i < s_len; ++i) {
		...
		dbu_add_jtag_sequence(1, NULL, 0, bit, NULL, 0);
	}
}

/* Move to the end state by queuing a sequence to clock into TMS */
static void dbu_state_move(void)
{
	...
	dbu_add_tms_sequence(&tms_scan, tms_scan_bits);
...
static void dbu_execute_scan(struct jtag_command *cmd)
{
	...
	if (cmd->cmd.scan->ir_scan) {
		if (tap_get_state() != TAP_IRSHIFT) {
			dbu_end_state(TAP_IRSHIFT);
			dbu_state_move();
		}
	} else {
		if (tap_get_state() != TAP_DRSHIFT) {
			dbu_end_state(TAP_DRSHIFT);
			dbu_state_move();
		}
	}

	dbu_end_state(cmd->cmd.scan->end_state);
	...
	if (tap_get_state() != tap_get_end_state()) {
		dbu_end_state(tap_get_end_state());
		dbu_state_move();
	}
...
static void dbu_pathmove(int num_states, enum tap_state *path)
{
	uint8_t tms0 = 0x00;
	uint8_t tms1 = 0xff;
	...
	for (int i = 0; i < num_states; i++) {
		if (path[i] == tap_state_transition(tap_get_state(), false))
			dbu_add_tms_sequence(&tms0, 1);
		else if (path[i] == tap_state_transition(tap_get_state(), true))
			dbu_add_tms_sequence(&tms1, 1);
		else {
	...
	dbu_end_state(tap_get_state());
}

static void dbu_execute_pathmove(struct jtag_command *cmd)
{
	LOG_DEBUG_IO("pathmove: %i states, end in %i",
		      cmd->cmd.pathmove->num_states,
	       cmd->cmd.pathmove->path[cmd->cmd.pathmove->num_states - 1]);

	dbu_pathmove(cmd->cmd.pathmove->num_states, cmd->cmd.pathmove->path);
}

static void dbu_stableclocks(unsigned int num_cycles)
{
	uint8_t tms = tap_get_state() == TAP_RESET;
	/* TODO: Perform optimizations? */
	/* Execute num_cycles. */
	for (unsigned int i = 0; i < num_cycles; i++)
		dbu_add_tms_sequence(&tms, 1);
}

static void dbu_runtest(unsigned int num_cycles)
{
	enum tap_state saved_end_state = tap_get_end_state();

	/* Only do a state_move when we're not already in IDLE. */
	if (tap_get_state() != TAP_IDLE) {
		dbu_end_state(TAP_IDLE);
		dbu_state_move();
	}
	dbu_stableclocks(num_cycles);

	/* Finish in end_state. */
	dbu_end_state(saved_end_state);

	if (tap_get_state() != tap_get_end_state())
		dbu_state_move();
}

static void dbu_execute_runtest(struct jtag_command *cmd)
{
	LOG_DEBUG_IO("runtest %u cycles, end in %i", cmd->cmd.runtest->num_cycles,
		      cmd->cmd.runtest->end_state);

	dbu_end_state(cmd->cmd.runtest->end_state);
	dbu_runtest(cmd->cmd.runtest->num_cycles);
}

static void dbu_execute_stableclocks(struct jtag_command *cmd)
{
	LOG_DEBUG_IO("stableclocks %u cycles", cmd->cmd.runtest->num_cycles);
	dbu_stableclocks(cmd->cmd.runtest->num_cycles);
}

static void dbu_execute_tms(struct jtag_command *cmd)
{
	LOG_DEBUG_IO("TMS: %u bits", cmd->cmd.tms->num_bits);
	dbu_cmd_dap_swj_sequence(cmd->cmd.tms->num_bits, cmd->cmd.tms->bits);
}

/* TODO: Is there need to call dbu_flush() for the JTAG_PATHMOVE,
 * JTAG_RUNTEST, JTAG_STABLECLOCKS? */
static void dbu_execute_command(struct jtag_command *cmd)
{
	switch (cmd->type) {
		case JTAG_SLEEP:
			dbu_flush();
			dbu_execute_sleep(cmd);
			break;
		case JTAG_TLR_RESET:
			dbu_flush();
			dbu_execute_tlr_reset(cmd);
			break;
		case JTAG_SCAN:
			dbu_execute_scan(cmd);
			break;
		case JTAG_PATHMOVE:
			dbu_execute_pathmove(cmd);
			break;
		case JTAG_RUNTEST:
			dbu_execute_runtest(cmd);
			break;
		case JTAG_STABLECLOCKS:
			dbu_execute_stableclocks(cmd);
			break;
		case JTAG_TMS:
			dbu_execute_tms(cmd);
			break;
...
static int dbu_execute_queue(struct jtag_command *cmd_queue)
{
	struct jtag_command *cmd = cmd_queue;

	while (cmd) {
		dbu_execute_command(cmd);
		cmd = cmd->next;
	}
...
static int dbu_speed(int speed)
{
	if (speed == 0) {
	...
	return dbu_cmd_dap_swj_clock(speed);
}

static int dbu_speed_div(int speed, int *khz)
{
	*khz = speed;
	return ERROR_OK;
}

static int dbu_khz(int khz, int *jtag_speed)
{
	*jtag_speed = khz;
	return ERROR_OK;
}
				...
				uint16_t *swo_prescaler)
{
	...
	if (!trace_enabled) {
		if (dbu_handle->trace_enabled) {
			retval = dbu_cmd_dap_swo_control(DAP_SWO_CONTROL_STOP);
			if (retval != ERROR_OK) {
				LOG_ERROR("Failed to disable the SWO-trace.");
				return retval;
			}
		}
		dbu_handle->trace_enabled = false;
		LOG_INFO("SWO-trace disabled.");
		return ERROR_OK;
	}

	if (!(dbu_handle->caps & INFO_CAPS_SWO_UART) &&
	    !(dbu_handle->caps & INFO_CAPS_SWO_MANCHESTER)) {
	...
	if (pin_protocol == TPIU_PIN_PROTOCOL_ASYNC_UART &&
	   (dbu_handle->caps & INFO_CAPS_SWO_UART)) {
	...
	retval = dbu_get_swo_buf_sz(&dbu_handle->swo_buf_sz);
	if (retval != ERROR_OK)
		return retval;

	retval = dbu_cmd_dap_swo_transport(DAP_SWO_TRANSPORT_DATA);
	if (retval != ERROR_OK)
		return retval;

	retval = dbu_cmd_dap_swo_mode(swo_mode);
	if (retval != ERROR_OK)
		return retval;

	retval = dbu_cmd_dap_swo_baudrate(*swo_freq, swo_freq);
	if (retval != ERROR_OK)
		return retval;
	...
	retval = dbu_cmd_dap_swo_control(DAP_SWO_CONTROL_START);
	if (retval != ERROR_OK)
		return retval;

	dbu_handle->trace_enabled = true;
 ...
 */
static int dbu_poll_trace(uint8_t *buf, size_t *size)
{
	uint8_t trace_status;
	size_t trace_count;
	...
	int retval = dbu_cmd_dap_swo_status(&trace_status, &trace_count);
	if (retval != ERROR_OK)
		return retval;
	...
	*size = trace_count < *size ? trace_count : *size;
	...
	do {
		size_t rb = 0;
		uint32_t packet_size = dbu_handle->packet_size - 4 /*data-reply*/;
		uint32_t remaining = *size - read_so_far;
		if (remaining < packet_size)
			packet_size = remaining;
		retval = dbu_cmd_dap_swo_data(
						packet_size,
						&trace_status,
						&rb,
						&buf[read_so_far]);
...
COMMAND_HANDLER(dbu_handle_info_command)
{
	if (dbu_get_version_info() == ERROR_OK)
		dbu_get_status();

	return ERROR_OK;
}

COMMAND_HANDLER(dbu_handle_cmd_command)
{
	uint8_t *command = dbu_handle->command;
	...
	int retval = dbu_xfer(dbu_handle, CMD_ARGC);

	if (retval != ERROR_OK) {
		LOG_ERROR("dbu command failed.");
		return ERROR_JTAG_DEVICE_ERROR;
	}
...
COMMAND_HANDLER(dbu_handle_vid_pid_command)
{
	...
	for (i = 0; i < CMD_ARGC; i += 2) {
		COMMAND_PARSE_NUMBER(u16, CMD_ARGV[i], dbu_vid[i >> 1]);
		COMMAND_PARSE_NUMBER(u16, CMD_ARGV[i + 1], dbu_pid[i >> 1]);
	}
	 ...
	 * dbu_vid_pid.
	 */
	dbu_vid[i >> 1] = dbu_pid[i >> 1] = 0;

	return ERROR_OK;
}

COMMAND_HANDLER(dbu_handle_backend_command)
{
	if (CMD_ARGC == 1) {
		if (strcmp(CMD_ARGV[0], "auto") == 0) {
			dbu_backend = -1; /* autoselect */
		} else {
			for (unsigned int i = 0; i < ARRAY_SIZE(dbu_backends); i++) {
				if (strcasecmp(dbu_backends[i]->name, CMD_ARGV[0]) == 0) {
					dbu_backend = i;
					return ERROR_OK;
				}
			}
...
COMMAND_HANDLER(dbu_handle_quirk_command)
{
	if (CMD_ARGC > 1)
		return ERROR_COMMAND_SYNTAX_ERROR;

	if (CMD_ARGC == 1)
		COMMAND_PARSE_ENABLE(CMD_ARGV[0], dbu_handle->quirk_mode);

	command_print(CMD, "dbu quirk workarounds %s",
				  dbu_handle->quirk_mode ? "enabled" : "disabled");
	return ERROR_OK;
}

static const struct command_registration dbu_subcommand_handlers[] = {
	{
		.name = "info",
		.handler = &dbu_handle_info_command,
		.mode = COMMAND_EXEC,
		.usage = "",
		.help = "show dbu info",
	},
	{
		.name = "cmd",
		.handler = &dbu_handle_cmd_command,
		.mode = COMMAND_EXEC,
		.usage = "",
		.help = "issue dbu command",
	},
	{
		.name = "vid_pid",
		.handler = &dbu_handle_vid_pid_command,
		.mode = COMMAND_CONFIG,
		.help = "the dbu vendor ID and product ID",
		.usage = "(vid pid)*",
	},
	{
		.name = "backend",
		.handler = &dbu_handle_backend_command,
		.mode = COMMAND_CONFIG,
	...
	{
		.name = "quirk",
		.handler = &dbu_handle_quirk_command,
		.mode = COMMAND_ANY,
...
static const struct command_registration dbu_command_handlers[] = {
	{
		.name = "dbu",
		.mode = COMMAND_ANY,
		.help = "perform dbu management",
		.usage = "<cmd>",
		.chain = dbu_subcommand_handlers,
	},
	COMMAND_REGISTRATION_DONE
};

static const struct swd_driver dbu_swd_driver = {
	.init = dbu_swd_init,
	.switch_seq = dbu_swd_switch_seq,
	.read_reg = dbu_swd_read_reg,
	.write_reg = dbu_swd_write_reg,
	.run = dbu_swd_run_queue,
};

//static const char * const dbu_transport[] = { "swd", NULL };

static struct jtag_interface dbu_interface = {
	.supported = DEBUG_CAP_TMS_SEQ,
	.execute_queue = dbu_execute_queue,
};

struct adapter_driver dbu_adapter_driver = {
	.name = "dbu",
//	.transports = dbu_transport,
	.transport_ids = TRANSPORT_SWD,
	.transport_preferred_id = TRANSPORT_SWD,
	.commands = dbu_command_handlers,
	.init = dbu_init,
	.quit = dbu_quit,
	.reset = dbu_reset,
	.speed = dbu_speed,
	.khz = dbu_khz,
	.speed_div = dbu_speed_div,
	.config_trace = dbu_config_trace,
	.poll_trace = dbu_poll_trace,

	.jtag_ops = &dbu_interface,
	.swd_ops = &dbu_swd_driver,
};
